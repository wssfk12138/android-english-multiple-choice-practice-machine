...
程序只会按构建时提供的顺序尝试受控镜像；从镜像清单取到更新时，程序会把
...
远程 ESQ 题库目录与程序更新通道相互独立。在真实、受控的目录发布前，官方题库
目录和受控镜像默认列表都保持为空，不伪造下载地址。用户手动填写的第三方目录
是与官方回退源隔离的替代来源，不会加入官方回退链。ESQ 下载后会校验目录声明的
